...
class CYButton:
    def __init__(self, master, text, command, position):
...
class CYtoolButtun:
    def __init__(self, master, text, command, position):
        self.btn = tk.Button(master = master, 
                                text=text, 
                                command=command,
                                highlightbackground=styles.app_bar_background)
        self.btn.place(x = position[0], y = position[1])
...
class CYImageButton:
    def __init__(self, master, image_path, label, size, command, position):
